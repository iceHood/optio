...
export async function execTaskInRepoPod(
  ...
  const script = [
    ...
    `fi`,
    // Task-level runtime deps (agent/skill/mcp requirements, per-task).
    // Uses hash-based caching: if the same install plan was already applied, skip.
    `if [ -n "\${OPTIO_TASK_RUNTIME:-}" ]; then`,
    `  TASK_RT_HASH=$(echo "\${OPTIO_TASK_RUNTIME}" | sha256sum | cut -c1-16)`,
    `  if [ ! -f "/home/agent/.optio-task-rt-\${TASK_RT_HASH}" ]; then`,
    `    echo "[optio] Installing task-level runtime deps (hash: \${TASK_RT_HASH})..."`,
    `    TASK_SYS=$(echo "\${OPTIO_TASK_RUNTIME}" | python3 -c "import json,sys; d=json.load(sys.stdin); print(' '.join(d.get('systemPackages',[])))" 2>/dev/null || echo "")`,
    `    TASK_NODE=$(echo "\${OPTIO_TASK_RUNTIME}" | python3 -c "import json,sys; d=json.load(sys.stdin); print(' '.join(d.get('nodePackages',[])))" 2>/dev/null || echo "")`,
    `    TASK_PY=$(echo "\${OPTIO_TASK_RUNTIME}" | python3 -c "import json,sys; d=json.load(sys.stdin); print(' '.join(d.get('pythonPackages',[])))" 2>/dev/null || echo "")`,
    `    TASK_SETUP=$(echo "\${OPTIO_TASK_RUNTIME}" | python3 -c "import json,sys; d=json.load(sys.stdin); [print(c) for c in d.get('setupCommands',[])]" 2>/dev/null || echo "")`,
    `    [ -n "\${TASK_SYS}" ] && { sudo apt-get update -qq 2>/dev/null && sudo apt-get install -y -qq \${TASK_SYS} 2>&1 | tail -3 || true; }`,
    `    [ -n "\${TASK_NODE}" ] && { npm install -g \${TASK_NODE} 2>&1 | tail -3 || true; }`,
    `    [ -n "\${TASK_PY}" ] && { pip install --break-system-packages \${TASK_PY} 2>&1 | tail -3 || true; }`,
    `    if [ -n "\${TASK_SETUP}" ]; then echo "\${TASK_SETUP}" | while IFS= read -r cmd; do [ -n "\${cmd}" ] && eval "\${cmd}" || true; done; fi`,
    `    touch "/home/agent/.optio-task-rt-\${TASK_RT_HASH}"`,
    `    echo "[optio] Task runtime deps installed"`,
    `  else`,
    `    echo "[optio] Task runtime deps already installed (cached: \${TASK_RT_HASH})"`,
    `  fi`,
    `fi`,
